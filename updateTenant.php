<? 
	...
	if ($_SERVER['REQUEST_METHOD'] == "POST")
	{
		...
		if (!($stmt)) {
			...
			$stmt->bind_param('sssbi', $_POST['firstName'], $_POST['lastName'], $_POST['email'], $has_room, $id);
			$stmt->execute();
			$stmt->store_result();
			$stmt->close();

			if ($has_room and $room_id != $old_room_id) {
				$stmt = $mysqli->stmt_init();
				...
				//next I need to handle the case where the user's room was removed.
			}
